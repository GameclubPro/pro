...
function openKeyboard(payload = 'home') {
  ...
  // URL WebApp; если есть код комнаты — добавляем ?join=XXXX (фронт это понимает)
  const webAppUrl = code ? `${PUBLIC_APP_URL}/?join=${encodeURIComponent(code)}` : PUBLIC_APP_URL;

  return {
    inline_keyboard: [
      // Один-единственный очевидный путь
...
// Возвращает последнюю по активности комнату, в которой состоит пользователь.
// Авторизация — через заголовок X-Telegram-Init-Data (как и в остальных REST ручках).
app.get('/api/self/active-room', async (req, res) => {
  try {
    ...
    // Берём «последнюю по активности» комнату, где он состоит
  ...
  return runPhaseOnce(roomId, async () =>
    withRoomLock(roomId, async () => {
      ...
      for (const p of updated.players) {
        io.to(`player:${p.id}`).emit('private:self', { roomPlayerId: p.id, role: p.role, alive: p.alive, roomCode: updated.code });
      }
...
async function validateNightTarget({ room, match, actor, role, target, nightNumber }) {
  ...
  switch (role) {
    ...
    case Role.JOURNALIST: {
      // Явный пропуск допустим
      if (!target) return { ok: true };
      if (target.id === self) return { ok: false, error: 'Нельзя расследовать себя' };
