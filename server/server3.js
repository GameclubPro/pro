...
// JWT настройки
const { JWT_SECRET = '', SESSION_TTL_SEC = '2592000' } = process.env;
...
// true только для обычной Мафии; Дон считается "мирным" в проверке шерифа.
const isSheriffDetectsMafia = (role) => role === Role.MAFIA;

/* ============================ JWT helpers (UPDATED) ============================ */
function signSession(user) {
  if (!JWT_SECRET) return null;
  const ttl = Math.max(3600, parseInt(SESSION_TTL_SEC, 10) || 2592000);
  const payload = { uid: Number(user.id) };
  // tid — необязательная метка происхождения (удобно в логах/аналитике)
...
function isLockError(e) {
  ...
  return (
    // Современные версии (классы из пакета)
    ...
    // Характерные тексты из разных версий redlock
    msg.includes('quorum') ||
...
if (USE_REDIS) {
  try {
    ...
    // Redlock — распределённые блокировки
    try {
      // Импортируем классы ошибок, чтобы корректно фильтровать конкуренцию
      ...
      redlock = new Redlock([redis], {
        retryCount: 1,              // чуть помягче конкуренция
        retryDelay: 150,            // мс
        retryJitter: 50,            // мс
        driftFactor: 0.01,
      ...
      redlock.on('error', (e) => {
        // Глушим ожидаемую конкуренцию (LockError/ExecutionError и их вариации)
    ...
    try {
      // современный пакет экспортирует RedisStore именованным
      const { RedisStore } = require('rate-limit-redis');
      redisRateStore = RedisStore;
    } catch (e1) {
      try {
        // старые версии экспортировали класс по умолчанию
...
const corsOptions = {
  origin: (origin, cb) => {
    if (!origin) return cb(null, true); // запросы без Origin — пускаем без CORS
...
// ✅ FIXED: корректная сборка WebApp URL через URL API (без двойного '?' и лишних '/')
function openKeyboard(payload = 'home') {
  const code = payload.startsWith('join-') ? payload.slice(5) : '';

  const u = new URL(PUBLIC_APP_URL); // корректно парсит и базовый URL, и существующие query-параметры
  if (code) u.searchParams.set('join', code);

  return {
    inline_keyboard: [
      [{ text: '🎮 Открыть комнату', web_app: { url: u.toString() } }],
    ],
  };
}
...
// ⬇️ Обновлённый /start: поддержка ?start=join-<CODE>
bot.start(async (ctx) => {
  try {
    ...
    if (m) {
      ...
      if (r.ok) {
        await ctx.reply(
          ...
          { reply_markup: openKeyboard(`join-${code}`) } // откроет WebApp уже с нужным кодом
        );
      } else {
        const map = {
...
function toPublicUser(u) {
  return {
    ...
    nativeId: u.nativeId ?? null,          // ← добавили
...
async function joinRoomByCodeViaBot(codeRaw, tgUser) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // Нельзя пускать новых в активную игру
...
/* ============================ Auth REST ============================ */
// Telegram verify (оставили как было; теперь тоже возвращает JWT если задан JWT_SECRET)
app.post('/auth/verify', async (req, res) => {
  ...
  const token = signSession(userRow); // может быть null, если не задан JWT_SECRET
  return res.json({ ok: true, user: tg, ...(token ? { token } : {}) });
});

// ✅ NEW: Нативный "гость" для RN-клиента — возвращает {ok, user, token}
app.post('/auth/native/guest', async (req, res) => {
  try {
    ...
    const token = signSession(user); // JWT с payload { uid, tid? }
    return res.json({ ok: true, user: toPublicUser(user), token });
  } catch (e) {
...
app.get('/avatar/:tgId', avatarLimiter, async (req, res) => {
  try {
    ...
    // ETag по file_path (достаточно стабилен до смены аватара)
...
// ✅ NEW: Универсальный аватар по userId (RN и TG клиенты рисуют одинаково)
...
// Уже JWT-or-Telegram (allowStale=true), совместимо с RN и TG
app.get('/api/self/active-room', async (req, res) => {
  try {
    const auth = await authEither(req, { allowStale: true }); // ← ключевой момент
...
/* ============================ Room REST (UPDATED AUTH) ============================ */
// create
app.post('/api/rooms', createLimiter, async (req, res) => {
  try {
    const auth = await authEither(req); // JWT или Telegram
    if (!auth.ok) return res.status(auth.http || 401).json({ error: auth.error });
...
// view room (просмотр без авторизации допустим, но если авторизован — показываем больше)
app.get('/api/rooms/:code', async (req, res) => {
  try {
    const { code } = req.params;
    const auth = await authEither(req, { allowStale: true }); // можно и без авторизации, но если есть — используем
    ...
    res.json({
      ...
      playersCount: room.players.length, // ← нечувствительный счётчик
      viewerIsOwner,
    });
  } catch (e) {
...
app.post('/api/rooms/:code/leave', async (req, res) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (restCount === 0) {
        cancelTimer(room.id); // гасим таймер перед удалением комнаты
...
app.post('/api/rooms/:code/to-lobby', async (req, res) => {
  try {
    ...
    // 🔔 NEW: after to-lobby, emit fresh private:self with role:null to every player
    ...
    await emitMafiaTeam(room.id); // сообщим пустой состав
...
// подключаем Redis-адаптер если доступен
if (USE_REDIS && socketRedisAdapter) {
  socketRedisAdapter(io).catch((e) => console.warn('Socket adapter init failed:', e?.message || e));
}

// 🔐 Аутентификация Socket.IO: сначала JWT (Authorization/Auth.token), затем Telegram initData (fallback)
...
// ===== Phase mutex per room (prevents double resolve in-process) =====
const phaseInFlight = new Set(); // roomId
...
io.on('connection', (socket) => {
  ...
  // запомним свои playerId (их может быть несколько в теории)
  ...
  socket.on('room:subscribe', async ({ code }) => {
    try {
      ...
      await emitMafiaTargets(room.id);
      await emitMafiaTeam(room.id); // актуальный состав мафии в maf:<roomId>
    } catch (e) {
  ...
  socket.on('night:act', async ({ code, targetPlayerId }, cb) => {
    try {
      ...
      // ✅ Снайпер может пропустить ход без траты патрона и без записи действия.
      // Это синхронизировано с тем, что SNIPER не учитывается в isNightReady().
  ...
  socket.on('room:leave', async ({ code }, cb) => {
    try {
      ...
      await withRoomLock(room.id, async () => {
        await prisma.$transaction(async (tx) => {
          // ищем участника по текущему пользователю
          ...
          // если после удаления никого не осталось — удаляем комнату
          const restCount = await tx.roomPlayer.count({ where: { roomId: room.id } });
          if (restCount === 0) {
            cancelTimer(room.id);
            await tx.room.delete({ where: { id: room.id } });
          ...
          // если уходил владелец — передаём овнерство первому по joinedAt
      ...
      // Отдаём ACK сразу, чтобы клиент мог мгновенно завершить UI/сокет
      ackOk(cb);

      // Актуализируем подписчиков комнаты и маф-каналы
      emitRoomStateDebounced(code);
      await rebuildMafiaRoom(room.id);
      await emitMafiaTargets(room.id);
  ...
  socket.on('disconnect', (reason) => {
    ...
    // планируем мягкую «уборку» игрока, если не осталось ни одного его сокета
    try {
...
const pendingAutoLeave = new Set(); // playerId -> true, чтобы не дублировать таймеры
function scheduleAutoLeaveOnDisconnect(playerId, delayMs = Math.max(0, parseInt(AUTO_LEAVE_GRACE_MS, 10) || 120000)) {
  ...
  const t = setTimeout(async () => {
    ...
    try {
      // Если за время грейса появился хоть один сокет игрока — ничего не делаем
      const sockets = await io.in(`player:${playerId}`).allSockets();
      if (sockets && sockets.size > 0) return;

      // читаем игрока и его комнату
      ...
      // ⚠️ В активной игре никого автоматически не удаляем —
      // ждём, пока игрок вернётся или хозяин решит сам.
      ...
      await withRoomLock(roomId, async () => {
        await prisma.$transaction(async (tx) => {
          // ещё одна проверка на «появился сокет» перед удалением
          ...
          // передаём овнерство при необходимости
...
async function publicRoomState(code) {
  ...
  if (room.status === Phase.VOTE) {
    ...
    vote.round = round;
    // alive, tally уже внутри voteProgress
  }
...
function composeRolesFor(n) {
  ...
  // ⚖️ Ваше требование для мини-лобби:
  // 4 игроков → 3 мирных + 1 мафия (без силовых ролей)
  if (n === 4) return [R.MAFIA, R.CIVIL, R.CIVIL, R.CIVIL];
  // 5 игроков → 2 мафии (Дон + Мафия), 1 доктор, 1 шериф, 1 мирный
  // и при этом шериф видит Дона как мирного (см. isSheriffDetectsMafia)
...
async function startGame(roomId) {
  return runPhaseOnce(roomId, async () =>
    withRoomLock(roomId, async () => {
      ...
      // Страховка: подчистим «хвосты» прошлой партии в этой комнате
      ...
      // 🔄 CHANGE: рассылаем единый приватный слепок через privateSelfState (включая userId)
...
// Учитываем блоки при готовности ночи
async function isNightReady(roomId, matchId, nightNumber) {
  ...
  // Важно: заблокированная проститукой мафия считается "сделала ход",
  // иначе ночь может зависнуть навсегда.
...
async function resolveNight(roomId) {
  return runPhaseOnce(roomId, () =>
    withRoomLock(roomId, async () => {
      ...
      if (!room) return;
      if (room.status !== Phase.NIGHT) return; // фаза могла смениться другим воркером
      ...
      // Инсайты Шерифа/Журналиста — только если они не заблокированы
      ...
      if (shAction?.targetPlayerId && !sheriffIsBlocked) {
        ...
        // 🕵️ Шериф не видит Дона как мафию — только "MAFIA"
      ...
      // Mafia votes (DON + MAFIA) except blocked actors
      ...
      if (after.status === Phase.ENDED) {
        ...
        // 🔔 Личные сигналы игрокам об исходе ночи (БЛОК/ЛЕЧЕНИЕ/ОХРАНА)
        try {
...
async function leadersOfRound1(roomId, dayNumber) {
  // Берём именно последнее событие «ничья первого раунда» за нужный день
...
async function validateNightTarget({ room, match, actor, role, target, nightNumber }) {
  ...
  switch (role) {
    ...
    case Role.SNIPER: {
      // Пропуск допустим (не тратит патрон), реальный выстрел — только по живой цели
      if (!target) return { ok: true };
...
// Тик-планировщик (каждую секунду проверяем «просроченные» фазы)
    ...
    try {
      ...
      for (const r of due) {
        try {
          ...
          // LockError игнорируем — другой воркер уже занимается комнатой
        }
      }
    } catch (e) {
...
async function cleanupRooms() {
  ...
  try {
    ...
    // NEW: лобби/завершённые комнаты без активных сокетов дольше idleMinutes → удаляем.
    // Это прикрывает кейс «свернули приложение, но в БД висят игроки», чтобы не было бесконечных комнат.
    ...
    for (const r of maybeIdle) {
      try {
        ...
        // Поддержка кластеров/Redis-адаптера: берём все сокеты во всех инстансах
