 ...
 * Режимы:
 *  - "shell" — домашний экран и секции
 *  - "game:<name>" — полноэкранная игра
 *
 * Поддерживаемые игры:
 *  - mafia | crocodile | associations | quiz | questions | truthordare | compatibility | choice | sketch
 */

// ⚙️ env-переменные (Vite): VITE_API_BASE / VITE_BOT_USERNAME
const API_BASE = import.meta.env?.VITE_API_BASE || "https://api.play-team.ru";
const BOT_USERNAME = (import.meta.env?.VITE_BOT_USERNAME || "PlayTeamBot").replace(/^@/, "");

// Пробрасываем имя бота в глобал (нужно Mafia.jsx)
...
const INVITE_STORAGE_KEY = `pt_consumed_invites_v1_${BOT_USERNAME}`;
...
function writeConsumedInvites(map) {
  try { localStorage.setItem(INVITE_STORAGE_KEY, JSON.stringify(map)); } catch {}
}
...
export default function App() {
  ...
  const [route, setRoute] = useState({ kind: "shell" }); // { kind:'shell' } | { kind:'game', name: 'mafia' | ... }
  ...
  // Доп. источник initData из URL (на случай запуска через deeplink в браузере)
  const initFromUrl =
  ...
  // Поддержка запуска через ?startapp=... → Telegram добавляет tgWebAppStartParam
  ...
  useEffect(() => { consumedInvitesRef.current = readConsumedInvites(); }, []);
  ...
  // Признак «почти точно открыто внутри Telegram»
  ...
  useEffect(() => {
    ...
    const pump = () => {
      ...
      if (id) { setResolvedInitData(id); return; }
      if (tries++ < 40) setTimeout(pump, 100); // максимум ~4с
    };
  ...
  /* ---------- Серверная верификация (по желанию) ---------- */
  useEffect(() => {
    if (!resolvedInitData) return;
    if (user?.id) return; // уже есть пользователь
  ...
  useEffect(() => {
    ...
    const code = m ? m[1].toUpperCase() : null;
    if (code && !isInviteConsumed(code) && code !== inviteCode) setInviteCode(code);
  }, [tg?.initDataUnsafe?.start_param, inviteCode, isInviteConsumed]);

  // разбор строки resolvedInitData
  useEffect(() => {
    if (!resolvedInitData) return;
    ...
    const code = m ? m[1].toUpperCase() : null;
    if (code && !isInviteConsumed(code) && code !== inviteCode) setInviteCode(code);
  }, [resolvedInitData, inviteCode, isInviteConsumed]);

  // ---- Delegation BackButton ----
  const backHandlerRef = useRef(null);
  const backProxyRef = useRef(null);
  const setBackHandler = (fn) => { backHandlerRef.current = typeof fn === "function" ? fn : null; };
  ...
  useEffect(() => {
    ...
    return () => { tg?.BackButton?.offClick?.(backProxyRef.current); };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setStatus(r.ok ? "✅ API доступен" : "❌ Ошибка API");
      } catch { if (!cancelled) setStatus("❌ Сервер недоступен"); }
    })();
    return () => { cancelled = true; };
  }, []);
  ...
  // ---- ФОЛБЭК автопоиска активной комнаты ----
  useEffect(() => {
  ...
  // ---- Автопереход в Мафию, если найден инвайт ----
  useEffect(() => { if (inviteCode) setRoute({ kind: "game", name: "mafia" }); }, [inviteCode]);

  // ---- Guard: не Telegram → показать кнопку "Открыть в Telegram"
    ...
    return (
      <div className="app" data-scheme={scheme} style={cssVars}>
        ...
        <div style={{ padding: 16, display: "grid", gap: 12 }}>
          ...
          <p style={{ margin: 0, opacity: 0.8 }}>Кажется, приложение запущено в браузере. Чтобы войти, открой его через Telegram.</p>
          <a className="dockCTA" href={deepLink} rel="noopener noreferrer"
             style={{ display: "inline-grid", placeItems: "center", textDecoration: "none", width: "100%", maxWidth: 360, height: 52,
                      borderRadius: 16, border: "1px solid color-mix(in srgb, var(--text) 12%, transparent)",
    ...
    <div className="app" data-scheme={scheme} style={cssVars}>
      ...
      {route.kind === "shell" ? (
        ...
        <GameCanvas>
          ...
          {route.name === "crocodile" && <Crocodile goBack={closeGame} onProgress={bumpProgress} setBackHandler={setBackHandler} />}
          {route.name === "associations" && <Associations goBack={closeGame} onProgress={bumpProgress} setBackHandler={setBackHandler} />}
          {route.name === "quiz" && <Quiz goBack={closeGame} onProgress={bumpProgress} setBackHandler={setBackHandler} />}
          {route.name === "questions" && <Questions goBack={closeGame} onProgress={bumpProgress} setBackHandler={setBackHandler} />}
          {route.name === "truthordare" && <TruthOrDare goBack={closeGame} onProgress={bumpProgress} setBackHandler={setBackHandler} />}
          {route.name === "compatibility" && <Compatibility goBack={closeGame} onProgress={bumpProgress} setBackHandler={setBackHandler} />}
          {route.name === "choice" && <Choice goBack={closeGame} onProgress={bumpProgress} setBackHandler={setBackHandler} />}
          {route.name === "sketch" && <SketchBattle goBack={closeGame} onProgress={bumpProgress} setBackHandler={setBackHandler} />}
        </GameCanvas>
      )}
    </div>
  );
      ...
      <div className="wrap">
        ...
        {section === "home" && (
          <div className="grid" role="list">
            <CategoryCard
              emoji="🎉" title="Мультиплеер" subtitle="комнаты и роли" participants="4–12"
              gradient="linear-gradient(135deg, rgba(14,165,233,.9), rgba(99,102,241,.9))"
              onClick={() => setSection("party")}
            />
            <CategoryCard
              emoji="🎮" title="На одном устройстве" subtitle="быстрые мини-игры" participants="2–12"
              gradient="linear-gradient(135deg, rgba(34,197,94,.95), rgba(20,184,166,.9))"
              onClick={() => setSection("local")}
            />
            <CategoryCard
              emoji="💞" title="Для влюблённых" subtitle="вопросы и челленджи" participants="2"
        ...
        {section === "party" && (
          <Section
            title="Мультиплеер" back={() => setSection("home")}
            items={[
        ...
        {section === "local" && (
          <Section
            title="На одном устройстве" back={() => setSection("home")}
            items={[
        ...
        {section === "love" && (
          <Section
            title="Для влюблённых" back={() => setSection("home")}
            items={[
...
function AvatarImg({ tgId, photoUrl, initials }) {
  ...
  useEffect(() => {
    if (tgId) { setSrc(`${API_BASE}/avatar/${tgId}`); triedProxy.current = true; }
    else { setSrc(photoUrl || ""); triedProxy.current = false; }
  }, [tgId, photoUrl]);

  if (!src) return <span>{initials}</span>;
  ...
  return (
    <img
      ...
      loading="eager"
      onError={() => {
        if (triedProxy.current && photoUrl) { setSrc(photoUrl); triedProxy.current = false; }
        else { setSrc(""); }
      }}
...
.shell .cardPill { position: absolute; top: 10px; right: 10px; z-index: 2; font-size: 12px; padding: 4px 8px; border-radius: 999px;
  background: rgba(0,0,0,.30); border: 1px solid rgba(255,255,255,.25); color: #fff; backdrop-filter: blur(6px); box-shadow: 0 6px 20px rgba(0,0,0,.12); display: inline-flex; gap: 4px; }
...
/* ===== GAME CANVAS ===== */
.gameCanvas { position: fixed; inset: 0; z-index: 1000; background: var(--bg, #000); display: block; overscroll-behavior: none; touch-action: manipulation; }
.gameStage { position: relative; inset: 0; width: 100%; height: 100%; overflow: auto; -webkit-overflow-scrolling: touch; overscroll-behavior: none; touch-action: manipulation; }

/* (перенос из "сырого" хвоста файла) */
html, body { margin: 0; padding: 0; }
:root { --tg-bg: #0b0f16; --tg-fg: #e8edf5; --tg-accent: #7aa7ff; }
.appWrap { min-height: 100%; }
        `,
      }}
    />
  );
