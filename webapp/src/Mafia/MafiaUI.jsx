...
/* =============================================================================
   === HUD =====================================================================
   Карточка HUD: код, фаза, этапы игры (GameStage/FinalBanner), кнопки
   ========================================================================== */
...
export function HUD({
  ...
  onRefresh,
  onLeave,
  canStart, // оставлено в сигнатуре для совместимости (старт у нас в центре сетки)
  onStart, // (не используется в HUD — см. PlayerGrid центр)
  endedLabel, // крупная надпись в финале
  // NEW: readiness в лобби для гостей (в HUD более не используется; кнопка — в центре сетки)
  iAmReady,
  onToggleReady,
}) {
  // «Только что стартовали» — короткий вход анимаций
  ...
  useEffect(() => {
    ...
    if (prev === "LOBBY" && phase !== "LOBBY") {
      ...
      // важно: сразу зафиксировать новую фазу, чтобы не триггерить повторно
  ...
  return (
    ...
    >
      {/* Верхняя строка: код и системные действия */}
      <div className="mf-hud-row">
        ...
        <div className="mf-hud-actions" role="group" aria-label="Действия">
          {/* Кнопка «Готов/Не готов» удалена из HUD — доступна только в центре сетки лобби */}
          <button
            className="mf-chip ghost"
            onClick={onRefresh}
            aria-label="Обновить"
      ...
      {phase !== "LOBBY" && (
        <div className="mf-hud-hint" role="note">
          {phase === "NIGHT" && "Ночь: выбери цель на сетке игроков"}
          {phase === "DAY" && "Днём обсуждаем — без действий"}
          {phase === "VOTE" && "Голос: нажми на игрока, за которого голосуешь"}
        </div>
      )}
    </section>
  );
...
export const TimerHUD = memo(
  function TimerHUD({ timer }) {
    const endsAtMs = toMs(timer?.endsAt) || 0;
    const serverAtMs = toMs(timer?.serverTime) || 0;

    // Ключ фазы — меняется, только когда меняется endsAt (начался новый этап)
    const animKey = endsAtMs;

    // Разница часов клиент↔сервер фиксируется на старт фазы
    const skewRef = useRef(0);
    useEffect(() => {
      skewRef.current = serverAtMs ? Date.now() - serverAtMs : 0;
    }, [animKey, serverAtMs]);

    // Длительность CSS-анимации фиксируем один раз на начало фазы
    ...
    // Тикер 1 Гц — только для текста MM:SS (прогресс — чистый CSS)
    ...
    const msLeft = Math.max(
      0,
      endsAtMs - (Date.now() - (skewRef.current || 0))
    );
    ...
    return (
      ...
      >
        ...
        <span
          className="mf-timer-bar"
          style={{ ["--msLeft"]: `${initialLeft}ms` }}
        >
          <i key={animKey} />
        </span>
      </div>
  ...
  (prev, next) => {
    const pe = toMs(prev?.timer?.endsAt);
    const ne = toMs(next?.timer?.endsAt);
    const pp = prev?.timer?.phase || "";
    const np = next?.timer?.phase || "";
    const pr = prev?.timer?.round || 1;
    const nr = next?.timer?.round || 1;
    return pe === ne && pp === np && pr === nr;
  }
);
...
export function GameStage({ phase, dayNumber, timer, animate = false }) {
  ...
  const current = String(phase || "LOBBY").toUpperCase();

  return (
    <div className={`mf-gamestage ${animate ? "mf-gs-appear" : ""}`}>
      <div className="mf-gs-top" aria-label="Этапы">
        {stages.map((s) => {
          const active = s.key === current;
          return (
            <div
              key={s.key}
              className={`mf-gs-chip ${active ? "active" : ""}`}
      ...
      <div className="mf-gs-bottom" aria-label="Таймер и день">
        <div className="mf-gs-timer">
          {timer ? <TimerHUD timer={timer} /> : null}
        </div>
        <div className="mf-gs-day" aria-label="Номер дня">
          <span className="label">День</span>
          <span className="val">{Math.max(0, dayNumber || 0)}</span>
        </div>
      </div>
    </div>
  );
...
/* =============================================================================
   === FINAL BANNER (крупная надпись в финале) =================================
   ========================================================================== */
export function FinalBanner({ label }) {
  const text = String(label || "Игра завершена").toUpperCase();
  const cls =
    /МАФИЯ/.test(text) ? "win-mafia" : /ГОРОД/.test(text) ? "win-city" : "";
  return (
    <div className={`mf-final-banner ${cls}`} role="status" aria-live="polite">
...
export function MenuViewV2({ busy, onCreate, onJoin, recentRooms = [] }) {
  const [code, setCode] = useState("");
  const [error, setError] = useState("");
  const inputRef = useRef(null);
  // Алфавит, совпадающий с серверным генератором (без 0/1/O/I)
  ...
  const normalized = useMemo(
    // На клиенте ограничиваем до 8 (как на сервере), остальное режем
    () =>
      (code || "")
        .toUpperCase()
  ...
  return (
    <section className="mf-menu v2" aria-label="Главное меню">
      {/* hero */}
      <header className="mf-menu-hero" role="banner">
        <div className="mf-menu-logo" aria-label="MAFIA">
          MAFIA
        </div>
        <p className="mf-menu-tagline">Играй в Telegram — быстро и красиво</p>
      </header>
      ...
      <div className="mf-menu-actions" role="group" aria-label="Действия">
        ...
        <div className="mf-join-inline">
          ...
          <input
            ...
            inputMode="text"
            maxLength={8}
            // подсказываем допустимый формат прямо браузеру
      ...
      {/* quick guide (bento) */}
      <section className="mf-menu-cards" aria-label="Как играть">
        <article className="mf-menu-card">
          <div className="ico" aria-hidden="true">
            🌘
          </div>
          <div className="title">Ночь</div>
          <p className="text">
            Мафия выбирает жертву; доктор лечит; шериф проверяет.
          </p>
        </article>
        <article className="mf-menu-card">
          <div className="ico" aria-hidden="true">
          ...
          <p className="text">
            Обсуждаем и готовимся к голосованию, действий нет.
          </p>
        </article>
        <article className="mf-menu-card">
          <div className="ico" aria-hidden="true">
          ...
          <div className="title">Голос</div>
          <p className="text">
            Выбираем подозреваемого. При ничьей — переголосование.
          </p>
        </article>
      </section>
